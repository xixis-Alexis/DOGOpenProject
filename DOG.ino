...
void loop()
{
  ...
  // and right value.
  proxSensors.read();
  uint8_t leftValue = proxSensors.countsFrontWithLeftLeds(); //6 à 0
  ...
  if (objectSeen)
  {
    ...
    if (leftValue < rightValue)
    {
      ...
      // make it more even.
        ...
        if (rightValue != 6)//à une distance inférieur à celle voulu
        {
    ...
    else if (leftValue > rightValue)
    {
      // The left value is greater, so turn to the left.
       if (leftValue != 6)//distance entre 0 à 1 et de plus de 25 cm
        {
    ...
    {
      //stop();
      if (distance > 15)//dans le cas où l'on est trop loin de l'objet
      {
